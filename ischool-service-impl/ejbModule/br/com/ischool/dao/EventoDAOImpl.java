...
public class EventoDAOImpl extends GenericDAOImpl<Evento,Long> implements EventoDAOLocal {
	...
	private QueryUtil queryConsultarEvento(Evento filtro, boolean count) {
		...
		if(!DadosUtil.isEmpty(filtro)) {
			...
			if(!DadosUtil.isEmpty(filtro.getIdEvento())) {
				queryUtil.adicionarFiltroExato("u.idEvento", filtro.getIdEvento());
			}	
			...
			if(!DadosUtil.isEmpty(filtro.getDataCadastro())) {
				...
				queryUtil.adicionarFiltroEntre("u.dataCadastro", filtro.getDataCadastro(),dataFim.getTime());
			}
	...
	private QueryUtil queryConsultarExisteEvento(Evento filtro, boolean count) {
		...
		if(!DadosUtil.isEmpty(filtro)) {
			...
			if(!DadosUtil.isEmpty(filtro.getCodigoEvento())) {
				queryUtil.adicionarFiltroExato("u.codigoEvento", filtro.getCodigoEvento());
			}					
	...
	public int consultarQtde(Evento filtro) throws DAOException {
		try{
			QueryUtil queryCount = queryConsultarExisteEvento(filtro, true);
